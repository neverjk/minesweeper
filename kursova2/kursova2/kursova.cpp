...
void Game();
void Menu() {
	mciSendString("open \"use\\VECTOR_GRAPHICS_DESTINE.mp3\" type mpegvideo alias mp3", NULL, 0, NULL); // музика
...
void CoutFieldX() { // справа від ігрового поля поле з усіма відкритими клітинками (додаткова функція)
	SetConsoleTextAttribute(hConsoleHandle, 8);
...
void YouLose() { // функція при програші
	mciSendString("close mp3", NULL, 0, NULL);
	...
	Sleep(3000);
	cout << "\n\n> go to menu" << endl;
	while (1) { // чекає, поки користувач натисне ентер
		char push = _getch();
		int push_int = static_cast<int>(push);
		if (push == 13) {
			PlaySound(".\\use\\FGBS(9).wav", NULL, SND_ASYNC);
...
void YouWin() { // функція при виграші
	mciSendString("close mp3", NULL, 0, NULL);
	PlaySound(".\\use\\FGBS(40).wav", NULL, SND_ASYNC);
	CoutAll();
	cout << "\tYOU WIN" << endl;
